# OrderServiceTest Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write 11 Mockito unit tests covering the `OrderService` checkout hot path and pay flow.

**Architecture:** Single test class using `@ExtendWith(MockitoExtension.class)`. All dependencies are mocked. The `self` field (self-injection proxy) is replaced with the real `orderService` instance via `ReflectionTestUtils.setField` so that `loadCartSnapshot()` and `persistOrder()` run for real, making the compensation logic in `checkout()` fully exercisable. `@Transactional` does not apply in unit tests — that is intentional; we are testing business logic, not transaction boundaries.

**Tech Stack:** JUnit 5, Mockito, AssertJ, Spring Test (`ReflectionTestUtils`)
...
| Action | Path |
|--------|------|
| Create | `src/test/java/com/example/flashsale/service/OrderServiceTest.java` |

---

## Task 1: Test skeleton + `loadCartSnapshot` tests
...
```
mvn test -Dtest="OrderServiceTest#loadCartSnapshot_emptyCart_throwsIllegalArgument+loadCartSnapshot_inactiveProduct_throwsIllegalArgument+loadCartSnapshot_activeProduct_doesNotThrow" --no-transfer-progress
```
...
```
git add src/test/java/com/example/flashsale/service/OrderServiceTest.java
git commit -m "test: OrderServiceTest skeleton + loadCartSnapshot edge cases"
```

---

## Task 2: `checkout()` tests

**Files:**
- Modify: `src/test/java/com/example/flashsale/service/OrderServiceTest.java`

Add the four methods below inside `OrderServiceTest`, after the `loadCartSnapshot` tests.

- [ ] **Step 1: Add the four `checkout()` test methods**
...
- [ ] **Step 2: Run the four checkout tests**

```
mvn test -Dtest="OrderServiceTest#checkout_lockTimeout_throwsRuntimeException_noCompensation+checkout_secondItemSoldOut_throwsSoldOutException_andCompensatesFirst+checkout_persistOrderFails_compensatesAllDeductions+checkout_success_returnsOrderResponse" --no-transfer-progress
```
...
```
git add src/test/java/com/example/flashsale/service/OrderServiceTest.java
git commit -m "test: checkout() edge cases — lock timeout, sold out, compensation, success"
```
...
Add the four methods below inside `OrderServiceTest`, after the checkout tests.

- [ ] **Step 1: Add the four `pay()` test methods**
...
- [ ] **Step 2: Run all 11 tests together**

```
mvn test -Dtest=OrderServiceTest --no-transfer-progress
```
...
```
git add src/test/java/com/example/flashsale/service/OrderServiceTest.java
git commit -m "test: pay() edge cases — success, not found, already paid, ownership check"
```
